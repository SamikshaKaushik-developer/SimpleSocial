...
def feed_page():
    ...
    if response.status_code == 200:
        # FIXED: Fast API returns a list directly, not dict["posts"]
        posts = response.json()
        ...
        for post in posts:
            ...
            with col1:
                st.markdown(
                    f"**{post.get('email', 'Unknown')}** • {post.get('created_at', '')[:10]}"
                )
            with col2:
...
if st.session_state.user is None:
    ...
    st.sidebar.title(f"👋 Hi {st.session_state.user.get('email', '')}!")
